...
| FEEL-09 | WP-07        | `done`        | All wholly WP-07-owned mapped groups pass and are enabled; after WP-12 additions the refreshed pinned report publishes 1,100/1,100 passing cases with zero failures, unsupported, missing, or errors. |
...
| DMN-05  | WP-09        | `done`        | Official `0084-feel-for-loops` passes 24/24 end to end, including `decision_014` consuming the `days in weekend` decision; the group remains enabled in the current 1,100-case pinned FEEL profile. |
