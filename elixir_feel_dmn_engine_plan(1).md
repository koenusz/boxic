...
| Phase 4 — Built-in Functions and Temporal Semantics | `done` | Lossless FEEL temporal and duration-kind types preserve precision and identity; `tzdata` supplies complete IANA-zone resolution and DST-aware date-time arithmetic. Numeric, string, conversion, aggregation, and core list built-ins are implemented. Every wholly WP-07-owned mapped group passes, and the strict profile passes 1,005/1,005. | None for the current Phase 4 scope; DMN-dependent property and iteration cases remain assigned to WP-09. |
| Phase 5 — Minimal DMN Execution                     | `in_progress` | WP-08 is complete: namespace-qualified definitions, input data, decisions, variables, information requirements, and literal expressions load into normalized structs with substantive validation. Dependency evaluation remains WP-09 work. | Resolve decision dependencies and pass relevant pinned upstream literal-expression cases through the formal DMN execution path. |
...
| WP-03        | `done`        | Five explicit statuses; exact typed-string loading; expected-error handling; Decimal, temporal, duration, nested, ordered, and unordered comparison; separate compatibility/coverage metrics; safe CLI selection; and versioned typed CSV/JSON schemas are tested. The current 1,005/1,005 strict report is tracked at `compatibility/feel-implemented-0dbcaf9b.{csv,json}`. | None for the current WP-03 scope. |
...
| WP-07        | `done` | Numeric/string groups pass 116/116; temporal constructors pass 273/273; equality passes 114/114; membership passes 327/327; all are enabled. Complete IANA resolution, DST-aware date-time duration arithmetic, general string conversion, and core list operations are implemented. Temporal properties pass 43/43 and iteration passes 23/23 within direct FEEL scope. | None for WP-07; DMN-dependent property/iteration residuals remain assigned to WP-09. |
...
| FEEL-09 | WP-07        | `done`        | All wholly WP-07-owned mapped groups pass and are enabled; the refreshed pinned report publishes 1,005/1,005 passing cases with zero failures, unsupported, missing, or errors. |
...
| WP-07/WP-09 | Cross-feature iteration | `0084-feel-for-loops` | `23/24`; all FEEL-owned numeric/date sequences pass, with one decision-dependency source remaining. | WP-09 supplies the required decision context; then enable the complete group. |
| WP-07 | Cross-type ranges and membership | `0072-feel-in` | `327/327`; enabled | Group remains fully passing. |
| WP-07/WP-09 | Temporal and range properties | `0074-feel-properties` | `43/53`; every direct FEEL temporal/property case passes, while ten range-context decisions are not discoverable through the current DMN model path. | WP-09 resolves and executes the ten DMN decisions; then enable the complete group. |
| WP-09/WP-12 | DMN-dependent structural expressions | `0016-some-every`, `0033-for-loops`, `0092-feel-lambda`, `1130-feel-interval` | Direct FEEL forms execute; remaining cases require decision dependencies, BKMs, decision services, boxed contexts, or namespaced DMN parsing. | Complete through the formal DMN work packages and publish in the DMN profile. |
...
1. complete DMN-01 through DMN-05 and the DMN-dependent structural groups through WP-09/WP-12;
2. begin WP-10 only after the minimal DMN model and upstream execution path are stable;
