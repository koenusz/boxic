...
| --------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Phase 0 — Repository and Fork Setup                 | `in_progress` | Umbrella, namespaces, baseline CI, update scripts, and the complete official upstream snapshot at `0dbcaf9b98bc3af4e36d44a7aed95e9e85703a13` are present. RodarFeel baseline/fork evidence is not recorded. | Review snapshot provenance, record the FEEL baseline, and satisfy the remaining Phase 0 deliverable. |
| Phase 1 — TCK Harness Foundation                    | `in_progress` | The loader discovers and normalizes the official namespaced corpus into 3,545 result entries; the runner, comparator, reporter, and Mix task exist. Explicit malformed-artifact records and complete compatibility reporting remain open. | Explicitly classify every pinned upstream case and publish complete compatibility/coverage reports. |
| Phase 2 — FEEL Core Compliance                      | `in_progress` | Decimal arithmetic, comparisons, boolean/null behavior, names, contexts, and lists have a unit-test baseline. `if then else` is not implemented, and no official upstream execution baseline has been published. | Implement the remaining core syntax and publish a pinned upstream core profile. |
...
| WP-01        | `in_review`   | Complete official upstream repository, pin file, upstream URL, update script, nested-git check, and CI verification script exist. | Review snapshot provenance/content verification before closing the work package. |
| WP-02        | `in_progress` | Official namespaced XML, model layout, scalar/list/component values, identifiers, labels, and compliance levels are normalized. | Add explicit malformed/missing artifact records rather than raising and maintain the reviewed enumeration baseline for future pins. |
...
| ------- | ------------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| TCK-01  | WP-01        | `done`        | Complete upstream repository vendored at `0dbcaf9b98bc3af4e36d44a7aed95e9e85703a13`; source URL recorded and fabricated fixtures removed. |
...
| TCK-05  | WP-02        | `in_review`   | Enumeration baseline records 150 official test documents, 154 DMN models, and 3,545 normalized result entries for the pin.                |
...
| CI-01   | WP-13        | `done`        | Fabricated fixture profiles, generated reports, CI invocation, and root test alias were removed; future compatibility reports use the official corpus. |
