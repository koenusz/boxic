...
| WP-03        | `done`        | Five explicit statuses; expected-error handling; Decimal, temporal, duration, nested, ordered, and unordered comparison; separate compatibility/coverage metrics; safe CLI selection; and versioned typed CSV/JSON schemas are tested. The current 175/175 strict report is tracked at `compatibility/feel-implemented-0dbcaf9b.{csv,json}`. | None for the current WP-03 scope. |
...
| WP-07 | Cross-type equality | `0068-feel-equality` | `95/114`; all structural range/unary-test equality cases pass, and all remaining cases are temporal. | Complete temporal equality, make the group pass, and enable it in the temporal profile. |
...
| WP-07/WP-09 | Cross-feature iteration | `0084-feel-for-loops` | `21/24`; the remaining cases are two temporal sequences and one decision-dependency source. | WP-07 implements temporal sequences and WP-09 supplies the required decision context; then enable the complete group. |
| WP-07 | Cross-type ranges and membership | `0072-feel-in` | `279/327`; every remaining case requires temporal literals or duration ordering. | Complete temporal membership, make the group pass, and enable it in the temporal profile. |
| WP-07 | Temporal properties | `0074-feel-properties` | `9/53`; the non-temporal context property case passes and remaining cases are temporal properties. | Complete temporal properties and enable the group. |
...
1. complete WP-07 temporal and built-in work exposed by `0068`, `0072`, `0074`, and `0084`;
2. complete DMN-01 through DMN-05 and the DMN-dependent structural groups through WP-09/WP-12;
3. begin WP-10 only after the minimal DMN model and upstream execution path are stable;
4. mature CI through CI-02 to CI-04 as upstream profiles become reliable.
