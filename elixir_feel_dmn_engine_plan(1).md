...
| Phase 2 — FEEL Core Compliance                      | `done`        | The pinned core profile passes 162/162 cases across conditionals, constants, arithmetic, contexts, lists, scalar equality, and boolean/null logic. The report is published at `compatibility/feel-implemented-0dbcaf9b.{csv,json}`. Cross-type range/unary-test and temporal equality cases from `0068` are explicitly assigned to WP-06/WP-07. | None for the current Phase 2 scope. |
...
| WP-06/WP-07 | Cross-type equality | `0068-feel-equality` | `86/114`; remaining cases cover range/unary-test value equality, zoned temporal values, temporal literals, and duration-kind equality. | Group passes completely after its structural and temporal dependencies are implemented, then enters the appropriate advanced profile. |
...
1. complete the WP-06/WP-07 structural and temporal dependencies exposed by `0068-feel-equality` and the remaining advanced exit-matrix groups;
2. use the published core baseline to drive FEEL-03 through FEEL-09 rather than expanding local fixtures as compatibility evidence;
