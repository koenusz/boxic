...
| WP-03        | `done`        | Five explicit statuses; exact typed-string loading; expected-error handling; Decimal, temporal, duration, nested, ordered, and unordered comparison; separate compatibility/coverage metrics; safe CLI selection; and versioned typed CSV/JSON schemas are tested. The current 1,029/1,029 strict report is tracked at `compatibility/feel-implemented-0dbcaf9b.{csv,json}`. | None for the current WP-03 scope. |
...
| FEEL-09 | WP-07        | `done`        | All wholly WP-07-owned mapped groups pass and are enabled; the refreshed pinned report publishes 1,029/1,029 passing cases with zero failures, unsupported, missing, or errors. |
...
| WP-07/WP-12 | Temporal and range properties | `0074-feel-properties` | `43/53`; every direct FEEL temporal/property case passes, while ten results use boxed DMN `context` expressions rather than literal-decision dependencies. | WP-12 implements boxed context execution; then enable the complete group. |
...
1. begin WP-10 now that the minimal DMN model and upstream literal-dependency path are stable;
