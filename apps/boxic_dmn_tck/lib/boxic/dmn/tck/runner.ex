defmodule Boxic.DMN.TCK.Runner do
  @moduledoc """
  Executes one TCK case through Boxic.DMN.
  """
  ...
  defp run_case(%Case{model_path: model_path} = test_case) do
    if not File.exists?(model_path) do
      ...
      case evaluate(test_case) do
        ...
        {:error, %Boxic.FEEL.Error{code: :unsupported_expression} = error} ->
          {:unsupported, error}
  ...
  defp evaluate(test_case) do
    with {:ok, %{source_profile: :dmn_1_5} = model} <- ModelLoader.load(test_case.model_path) do
      if test_case.metadata[:case_type] == "decisionService" do
        ...
        Boxic.DMN.evaluate(model, test_case.decision_name, test_case.inputs)
      end
    end
  end
end
