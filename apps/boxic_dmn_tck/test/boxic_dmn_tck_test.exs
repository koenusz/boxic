defmodule Boxic.DMN.TCKTest do
  use ExUnit.Case
  ...
  test "expected-error results pass only when evaluation returns an error" do
    ...
    assert %{status: :passed, error: %Boxic.FEEL.Error{}} = Runner.execute(test_case)
  ...
  test "DMN implemented profile is an explicit passing decision-table baseline" do
    ...
    assert result.summary.total == 1_485
    assert result.summary.passed == 1_485
    assert result.summary.failed == 0
    assert result.summary.error == 0
    assert result.summary.disabled == 0

    assert length(TCK.profile_groups("dmn", "implemented")) == 69
  end
  ...
  defp temporary_corpus_root do
    root =
      Path.join(
        System.tmp_dir!(),
        "boxic-tck-#{System.unique_integer([:positive, :monotonic])}"
      )
